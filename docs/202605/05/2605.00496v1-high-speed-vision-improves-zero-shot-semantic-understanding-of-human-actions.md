---
title: High-Speed Vision Improves Zero-Shot Semantic Understanding of Human Actions
title_zh: 高速视觉提升人类动作的零样本语义理解
authors: "Yongpeng Cao, Yuji Yamakawa"
date: 2026-05-01
pdf: "https://arxiv.org/pdf/2605.00496v1"
tags: ["query:vla"]
score: 6.5
evidence: 人机交互中动作的零样本语义理解
tldr: 本研究探讨了时间分辨率对高速人类动作零样本语义理解的影响。针对剑道等快速且细微的动作，提出了一种结合预训练视频语言模型与大语言模型推理的免训练流水线。实验证明，提高帧率（如120Hz）能显著提升语义可分性和识别稳定性，为高速视觉在复杂动作理解中的应用提供了重要依据。
source: arxiv
selection_source: fresh_fetch
figures_json: "[{\"url\": \"assets/figures/arxiv/2605.00496v1/fig-001.webp\", \"caption\": \"Fig. 4: Similarity Matrices for Full Kendo Actions with tracking results overlapped.\", \"page\": 5, \"index\": 1, \"width\": 1052, \"height\": 351}, {\"url\": \"assets/figures/arxiv/2605.00496v1/fig-002.webp\", \"caption\": \"Fig. 5: Similarity Matrices for Partially Observed Actions.\", \"page\": 5, \"index\": 2, \"width\": 1052, \"height\": 274}, {\"url\": \"assets/figures/arxiv/2605.00496v1/fig-003.webp\", \"caption\": \"Fig. 3: Similarity Matrices for Full Kendo Actions at Different Frame Rates.\", \"page\": 5, \"index\": 3, \"width\": 1052, \"height\": 338}, {\"url\": \"assets/figures/arxiv/2605.00496v1/fig-004.webp\", \"caption\": \"Fig. 2: Example of High-Speed Kendo Action Video Frame with and without Joints Tracking Results Overlaid.\", \"page\": 3, \"index\": 4, \"width\": 512, \"height\": 274}, {\"url\": \"assets/figures/arxiv/2605.00496v1/fig-005.webp\", \"caption\": \"Fig. 1: System Framework.\", \"page\": 2, \"index\": 5, \"width\": 430, \"height\": 535}]"
motivation: 针对快速且难以标注的动作，研究时间分辨率如何影响零样本语义理解的准确性。
method: 提出一种结合预训练视频语言模型进行语义表示，并利用大语言模型进行成对动作比较的免训练流水线。
result: 实验表明，120Hz的高时间分辨率在零样本设置下显著提高了动作的语义可分性和表示稳定性。
conclusion: 高速视觉感知能有效增强对快速动作的语义理解能力，是提升免训练动作识别性能的关键因素。
---

## 摘要
从视觉观测中理解人类动作对于人机交互至关重要，特别是在需要对不熟悉或难以标注的动作进行语义解释时。在快速且较少见的活动场景中，为监督学习收集充足的标注数据具有挑战性，这使得零样本方法成为无需特定任务训练即可进行语义理解的实用替代方案。虽然大规模预训练模型的最新进展使这种零样本推理成为可能，但时间分辨率的影响，特别是对于快速和细粒度运动的影响，仍未得到充分探索。在本研究中，我们调查了时间分辨率如何影响高速人类动作的零样本语义理解。我们以剑道作为快速且微妙运动模式的代表性案例，提出了一种无需训练的流水线，该流水线结合了用于语义表示的预训练视频-语言模型和基于大语言模型的成对动作比较推理。通过在多个帧率（120 Hz、60 Hz 和 30 Hz）下的受控实验，我们证明了更高的时间分辨率显著提高了零样本设置下的语义可分性。我们进一步分析了在全观测和部分观测场景下，基于追踪的人类关节信息的作用。使用最近类原型策略的定量评估表明，高速视频为快速动作提供了更稳定且更具解释性的语义表示。这些发现强调了时间分辨率在无需训练的动作识别中的重要性，并表明高速感知可以增强语义理解能力。

## Abstract
Understanding human actions from visual observations is essential for human--robot interaction, particularly when semantic interpretation of unfamiliar or hard-to-annotate actions is required. In scenarios such as rapid and less common activities, collecting sufficient labeled data for supervised learning is challenging, making zero-shot approaches a practical alternative for semantic understanding without task-specific training. While recent advances in large-scale pretrained models enable such zero-shot reasoning, the impact of temporal resolution, especially for rapid and fine-grained motions, remains underexplored.   In this study, we investigate how temporal resolution affects zero-shot semantic understanding of high-speed human actions. Using kendo as a representative case of rapid and subtle motion patterns, we propose a training-free pipeline that combines a pre-trained video-language model for semantic representation with large language model-based reasoning for pairwise action comparison. Through controlled experiments across multiple frame rates (120 Hz, 60 Hz, and 30 Hz), we show that higher temporal resolution significantly improves semantic separability in zero-shot settings. We further analyze the role of tracking-based human joint information under both full and partial observation scenarios. Quantitative evaluation using a nearest-class prototype strategy demonstrates that high-speed video provides more stable and interpretable semantic representations for fast actions. These findings highlight the importance of temporal resolution in training-free action recognition and suggest that high-speed perception can enhance semantic understanding capabilities.