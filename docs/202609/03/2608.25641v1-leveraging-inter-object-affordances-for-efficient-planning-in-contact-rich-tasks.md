...
title_zh: 利用物体间可供性实现接触丰富任务中的高效规划
...
tldr: 传统任务与运动规划方法通常假设简化对象模型，忽略接触丰富任务的关键物理属性，导致规划成功率低且规划速度慢。本文提出统一任务与运动规划（U-TAMP），利用视觉语言模型生成物体间可供性抽象，用于刻画抓取和支撑等物理交互约束。在模拟厨房整理任务中，该方法规划成功率显著提升，规划时间缩短一至两个数量级。该工作为涉及异质物体的接触丰富任务提供了高效规划的新思路。
source: arxiv
...
motivation: 传统任务与运动规划假设简化对象模型，忽略关键物理属性，导致接触丰富任务规划成功率低且速度慢。
method: 提出U-TAMP，利用视觉语言模型生成物体间可供性抽象，刻画抓取和支撑等物理约束以丰富规划域。
result: 在模拟厨房整理任务中，规划成功率显著提升，规划时间缩短一至两个数量级。
conclusion: 该方法为涉及异质物体的接触丰富任务提供了高效规划新思路，并优于现有基线方法。
---

## 摘要
传统任务与运动规划（TAMP）方法主要侧重于定义动作序列以及执行长时程任务所需的几何和运动学约束。然而，它们在现实环境中的适用性有限，因为它们通常假设简化的物体模型，忽略了对于成功执行接触丰富任务至关重要的关键物理属性。此外，它们通常在运动规划过程中使用亚符号推理，这大大增加了规划时间并降低了整体成功率。我们提出一种方法，利用TAMP方法，定义以对象为中心的执行约束抽象，称为统一TAMP（U-TAMP），以执行涉及具有异质形状、尺寸和材料的物体之间交互的机器人任务。使用视觉语言模型（VLM），我们生成物体间可供性抽象，用于表征接触丰富任务中物体之间的物理交互约束，例如抓取和支撑约束。这些约束用于丰富U-TAMP规划域，以处理具有可变物理属性的物体。我们在模拟厨房桌面整理场景中进行实验，并将结果与原始U-TAMP以及一种最先进的基于VLM的规划器（该规划器利用关于物体可供性的常识知识来生成计划）进行比较。与其他方法相比，我们的方法实现了显著更高的规划成功率，并将规划时间缩短了一到两个数量级。
